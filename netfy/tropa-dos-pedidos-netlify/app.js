...
function connectLiveUpdates(){
  clearInterval(connectLiveUpdates.timer);
  connectLiveUpdates.timer=setInterval(()=>{
    if(document.visibilityState==='hidden')return;
    loadStore();
    if($('#admin').classList.contains('visible')&&$('#admin').classList.contains('authenticated'))loadAdmin();
  },5000);
}
